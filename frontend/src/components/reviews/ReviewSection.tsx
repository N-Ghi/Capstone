...
export default function ReviewSection({ experienceId }: ReviewSectionProps) {
  ...
  const fetchReviews = useCallback(
    async (page: number = 1) => {
      ...
      try {
        ...
        setError(t("section.loadError"));
      } finally {
        setLoading(false);
      }
    },
    [experienceId, rating, user?.id, t]
  );
  ...
  const handleCreate = async (data: ReviewFormData | ReviewPatchData) => {
    ...
    <div className={styles.section}>
      ...
      {loading ? (
        ...
        <div className={styles.empty}>
          ...
          <p className={styles.emptyText}>{t("section.empty")}</p>
        </div>
      ) : (
        <>
              ...
              if (isEditing) {
                return (
                  <div key={review.id} className={styles.editingWrap}>
                    <ReviewFormInline
                      experienceId={experienceId}
                      initial={{ id: review.id, rating: review.rating, comment: review.comment }}
                      onSave={handleUpdate}
